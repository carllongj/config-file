...
class ReplaceContext(object):
    ...
    # 获取当前的模块名称,主要是通过此来添加哪些 package 需要添加日志
...
class ReCommandHandler(CommandHandler):
    """
    RE 命令替换处理器
    """
    ...
    def register(self, sub_parser) -> None:
        re_sub = sub_parser.add_parser(self.command_name, help="替换文件内容处理", aliases=["replace"])
    ...
    def call(self, namespace) -> None:
        ReplaceContext(namespace.env, namespace.path).handle()
